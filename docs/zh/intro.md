...
没有进行中的 PRIMARY 时，每次提交仍会重新解析，以便拿到远端可能更新后的内容。同一 URL 的并发提交则共享一次进行中的执行。

### 异步任务队列
任务基于优先级队列并发执行，同优先级按 FIFO 顺序处理，最大并发数可通过配置调整。未开始执行的任务支持取消。

### 媒体文件管理
解析结果可包含多个媒体项（图片、视频等）。Content Hive 自动下载每一个媒体文件，按结构化路径存储：`/config/data/media/{platform}/{author}/{content_id}/`。MIME 类型自动检测，视频封面图自动提取。
