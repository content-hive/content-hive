...
class TimestampMixin:
    created_at: Mapped[datetime] = mapped_column(
        AwareDatetime, default=lambda: datetime.now(UTC), nullable=False
    )
...
class User(Base, TimestampMixin):
    ...
    status: Mapped[UserStatus] = mapped_column(
        SQLEnum(UserStatus), default=UserStatus.INACTIVE, nullable=False
    )
    ...
    platforms: Mapped[list["Platform"]] = relationship(
        "Platform", secondary="user_platforms", back_populates="users"
    )
    authors: Mapped[list["Author"]] = relationship(
        "Author", secondary="user_authors", back_populates="users"
    )
    parse_results: Mapped[list["ParseResult"]] = relationship(
        "ParseResult", secondary="user_parse_results", back_populates="users"
    )
    tasks: Mapped[list["MainTask"]] = relationship(
        "MainTask", back_populates="user", cascade="all, delete-orphan"
    )
...
class Platform(Base, TimestampMixin):
    ...
    users: Mapped[list["User"]] = relationship(
        "User", secondary="user_platforms", back_populates="platforms"
    )
    authors: Mapped[list["Author"]] = relationship(
        "Author", back_populates="platform", cascade="all, delete-orphan"
    )
    parse_results: Mapped[list["ParseResult"]] = relationship(
...
class Author(Base, TimestampMixin):
    ...
    # Relationships
    users: Mapped[list["User"]] = relationship(
        "User", secondary="user_authors", back_populates="authors"
    )
...
class ParseResult(Base, TimestampMixin):
    ...
    state: Mapped[ParserResultStatus] = mapped_column(
        SQLEnum(ParserResultStatus), nullable=False
    )
    ...
    platform: Mapped["Platform"] = relationship(
        "Platform", back_populates="parse_results"
    )
    author: Mapped["Author"] = relationship("Author", back_populates="parse_results")
    users: Mapped[list["User"]] = relationship(
        "User", secondary="user_parse_results", back_populates="parse_results"
    )
    tasks: Mapped[list["MainTask"]] = relationship(
        "MainTask", back_populates="parse_result"
    )
...
class ParseResultMedia(Base):
    ...
    parse_result: Mapped["ParseResult"] = relationship(
        "ParseResult", back_populates="media_list"
    )
...
class MainTask(Base, TimestampMixin):
    ...
    role: Mapped[TaskRole | None] = mapped_column(
        SQLEnum(TaskRole), nullable=True
    )  # primary, linked, reused
    ...
    parse_result: Mapped[Optional["ParseResult"]] = relationship(
        "ParseResult", back_populates="tasks"
    )
...
class SubTask(Base, TimestampMixin):
    ...
    main_task_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("main_tasks.id", ondelete="CASCADE"), nullable=False
    )
    ...
    depends_on: Mapped[Optional["SubTask"]] = relationship(
        "SubTask", remote_side=[id], foreign_keys=[depends_on_id]
    )
