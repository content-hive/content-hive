...
class UserService:
    ...
    def create_user(
        ...
        with UserDAO() as dao:
            ...
            user_id = dao.create_user(
                username, password_hash, email, is_admin, created_by
            )
    ...
    def change_user_password(self, user_id: int, new_password: str) -> bool:
        ...
        with UserDAO() as dao:
            ...
            result = dao.update_user_password(
                user.id, new_password_hash, force_password_change=False
            )
    ...
    def reset_user_password(self, user_id: int) -> str:
        ...
        with UserDAO() as dao:
            ...
            new_password_hash = secret_manager.hash_password(new_password)
            result = dao.update_user_password(
                user.id, new_password_hash, force_password_change=True
            )
            dao.revoke_all_user_sessions(user.id)
            if not result:
    ...
    def list_users(self) -> list[UserProfileResponse]:
        ...
        with UserDAO() as dao:
            ...
            return [
                UserProfileResponse.from_entities(user, profile)
