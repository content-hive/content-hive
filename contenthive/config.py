...
class Settings(BaseSettings):
    ...
    plugins_repo_url: str = os.getenv(
        "PLUGINS_REPO_URL", "https://github.com/content-hive/plugins.git"
    )
    plugins_repo_ref_type: str = os.getenv(
        "PLUGINS_REPO_REF_TYPE", "branch"
    )  # branch, tag, commit
    ...
    download_max_retries: int = Field(
        default=int(os.getenv("DOWNLOAD_MAX_RETRIES", "3")), ge=0
    )
